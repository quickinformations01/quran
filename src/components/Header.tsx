...
import { Settings, Volume2, MapPin } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#064E3B] text-white border-b border-[#D4AF37]/30 shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-2">
        {/* Logo & App Title -> Clicking opens Home Grid */}
        <div 
          onClick={() => setActiveTab('home')} 
          className="flex items-center gap-3 cursor-pointer group"
          title="Go to Home Grid"
        >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          {currentlyPlayingAudio && (
            <button
              onClick={onStopAudio}
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[#D4AF37] text-[#064E3B] font-bold text-xs animate-pulse shadow cursor-pointer"
          ...
          <button
            ...
            className="hidden xs:flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-[#D4AF37]/20 border border-[#D4AF37]/40 text-[#D4AF37] text-xs font-semibold cursor-pointer"
            title="Fiqh Setting"
          >
